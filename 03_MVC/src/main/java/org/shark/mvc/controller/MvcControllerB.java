...
public class MvcControllerB {
  ...
  //----- 요청 주소: /b/detail?bid=1&code=detail&url=abc  세미
  @RequestMapping("/detail")
  public String methodB(@RequestParam(name = "bid") int bid   //----- @RequestParam 어노테이션으로 요청 파라미터를 처리합니다.
  ...
  //----- 요청주소 : /b/regist?title=제목&hit=0
  @RequestMapping("/regist")
  public String methodC(BoardDTO board) {  //----- 커맨드 객체를 이용한 요청 파라미터 처리 : Constructor 또는 Setter를 이용해서 요청 파라미터를 BoardDTO의 멤버 변수에 저장합니다.
  ...
  //----- 요청 주소 : /b/boards/1  (마지막 1은 경로에 포함된 값으로 경로 변수(Path Variable)라고 합니다. 파이널
  @RequestMapping("/boards/{bid}")  //------------------------------- {bid} : 경로 변수 bid
  public String methodD(@PathVariable(name = "bid") int bid) {  //-- name 을 생략하면 변수 이름과 같은 경로 변수를 받아옵니다.
    System.out.println("bid : " + bid);  
